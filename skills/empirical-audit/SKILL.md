...
description: Systematic five-audit empirical research review. Runs code, performs cross-language replication, checks directory structure, output automation, and econometrics. Produces a formal audit report. Use when asked to audit, replicate, or verify a research project's empirical validity — not for manuscript writing feedback (use /review-paper for that).
allowed-tools: Read, Write, Bash, Glob, Grep
argument-hint: "[project-root-path] [--all] [--audits 1,2,5] [--no-replication]"
source: mixtapetools
author: Scott Cunningham
adapted_by: Matthew Mair
adaptation: substantial
upstream_url: https://github.com/scunning1975/MixtapeTools/blob/main/personas/referee2.md
...
**Always ask before auditing. Never silently skip an audit.**

Audit 2 (cross-language replication) costs more than the other four combined —
it writes and runs a full parallel implementation in a second language. It is
the audit worth gating.

### Flags (skip the gate)

| Flag | Effect |
|---|---|
| `--all` | Run all five audits at full intensity; no prompt |
| `--audits 1,2,5` | Run exactly those audits; no prompt |
| `--no-replication` | Shorthand for every audit except 2 |

When a flag is present, skip straight to the audits and record the selection in
the report header as usual. With no flag, run the gate below.

### Natural-language invocation

The flags are conveniences, not requirements. Colloquial phrasing must work
identically — treat the following as equivalent to the flags above:

| The user says | Treat as |
|---|---|
| "all stages", "all audits", "all five", "everything", "full audit" | `--all` |
| "audits 1, 2 and 5", "just the code and econometrics audits", "1 2 5" | `--audits 1,2,5` |
| "skip the replication", "no cross-language", "all but 2" | `--no-replication` |
| "the usual", "your call", "whatever you recommend" | run the gate, then proceed with the recommendation |

**Vocabulary warning — "stage" is overloaded in this skill.** It means the
*project's* lifecycle stage (R&R, pre-submission, early stage) in PHASE 0's
suggestion table, but users commonly say "stage" to mean one of the five
*audits*. Resolve by context:

- "all stages" / "run every stage" / "stage 2" → they mean **audits**
- "what stage is this project" / "early stage" → they mean **project stage**

When genuinely ambiguous, ask — one short question is cheaper than running the
wrong scope. Never assume "stages" refers to project lifecycle when it appears
alongside a count or a number ("all five stages", "stages 1 and 3").

### The gate

1. **Detect stage.** If `~/.claude/private/PROJECTS.md` exists, look up this
   project's row and read its Status column. If the file does not exist, or the
   project is not listed, proceed with **no suggestion** — state that the stage
   is unknown and ask. Do not guess a stage.

2. **Present the scope table:**

```
EMPIRICAL AUDIT — SCOPE

Project: [name]
Stage:   [status from PROJECTS.md, or "unknown — not in registry"]

  #  Audit                          Cost      Suggested
  1  Code Audit                     low       [✓/—]
  2  Cross-Language Replication     VERY HIGH [✓/—]
  3  Directory & Repl. Package      low       [✓/—]
  4  Output Automation              low       [✓/—]
  5  Econometrics Audit             medium    [✓/—]

RECOMMENDED: [list]

[One paragraph: why this set, given the stage.]

Run the recommended set? Or reply with the audits you want:
  "all"        every audit at full intensity
  "1 2 5"      just those
  "all but 2"  everything except cross-language replication
```

3. **Wait for an answer.** Accept the recommendation, an explicit list, `all`,
   `all but N`, or a stage name to recalibrate against.

4. **Record the selection in the report header** — which audits ran, which did
   not, and why. A skipped audit must be visible in the output.

### Suggestion basis

| Stage | Suggest | Reasoning to state |
|---|---|---|
| R&R | 1, 2, 4, 5 | Headline estimates load-bearing; referees re-read tables |
| Pre-submission | 1, 2, 3, 5 | Last chance to catch a wrong number before it ships |
| Framing stage | 1, 5 | Specification still moving; structure churns |
| Early stage | 1 | Replicating code that may be abandoned is waste |
| Replication package | 3, 4 | Econometrics already refereed; does the package *run*? |
| Single script / quick check | 1 | Narrow scope, narrow audit |
| Unknown / not in registry | none — ask | State the stage is unknown, present the bare table |

Stage labels match the Status vocabulary in `private/PROJECTS.md`. These are
suggestions, not defaults — the user's answer always wins.

### Always recommend Audit 2 when

- A result changed unexpectedly after a data or specification change
- The estimate under review is the paper's headline number
- The user asks for replication

Say so explicitly when any of these applies, even if the stage table suggests
otherwise. The user can still decline, but the recommendation must be visible.
...
2. **Create a replication script** in the other language:
   - If primary is **R** → create a Python replication script
   - If primary is **Python** → create an R replication script
...
**Prior replication scripts — reuse carefully.**

If `code/replication/` already contains scripts from an earlier round, do **not**
silently reuse them, and do **not** read them before writing a new replication.

The entire justification for this audit's cost is orthogonality: an independently
written implementation fails differently than the original. Reading the prior
script anchors the new one to the same choices, and re-running it is not a
replication at all — it is a regression test. Both are useful; they answer
different questions, and the report must not conflate them.

When prior scripts exist, check whether the author's code has changed since (git
log on the analysis scripts, or file mtimes if the project is not versioned), then
present the choice:

```
PRIOR REPLICATION FOUND

  code/replication/referee2_replicate_main_results.py   (Round 1, 2026-03-14)
  Author code changed since: YES — code/R/03_analysis.R modified 2026-04-02

  A  Independent re-derivation   Write a fresh replication WITHOUT reading the
                                 prior script. Preserves orthogonality. Costly.
  B  Regression check            Re-run the existing script against current
                                 author code. Cheap. NOT a replication — it
                                 only detects change since Round 1.
  C  Update existing             Revise the prior script to match the changed
                                 specification. Middle cost, orthogonality
                                 already spent in Round 1.

RECOMMENDED: A — the author's specification changed since the prior replication,
so the old script no longer targets the current model. A stale script can produce
a spurious match or a spurious mismatch, and either is worse than no check.
```

Suggest **A** when the author's code has changed, or when the estimate under
review is the headline number. Suggest **B** when nothing has changed and the
user wants a cheap confirmation that results still reproduce. Never suggest B as
a substitute for a first-round replication.

**Label the result in the report.** "Replication matched" means something
different under A than under B, so state which was run:

- Under A: *"Independent replication (Python) matched R results to 8 decimal places."*
- Under B: *"Regression check: prior Round 1 replication script re-run against
  current author code; results unchanged. This is not an independent replication."*
...
- [ ] **Relative paths**: Are ALL file paths relative to the project root? Absolute paths (`C:\Users\...` or `/Users/<user>/...`) are automatic failures.
- [ ] **Naming conventions**:
  - Variables: Are names informative? (`treatment_intensity` not `x1`)
  - Datasets: Do names reflect contents? (`county_panel_2000_2020.csv` not `data2.csv`)
...
- [ ] **Data dictionary**: Does `data/dictionary.md` exist? Run `code/R/00_validate_dictionary.R` to check whether it covers all columns in `data/clean/`. Flag any undocumented variables as a deficiency.
...
=================================================================
              ...
              Date: YYYY-MM-DD
              Scope: [audits run] | Skipped: [audits not run]
=================================================================
...
**Scope.** Audits run: [list]. Not run: [list, with the reason given at
selection — stage-based suggestion, user request, or flag]. A skipped audit
is not a passed audit; state plainly what was not checked.
...
```latex
\begin{frame}{Main DiD Estimate Matches Across Both Languages}
\begin{table}
\centering
\begin{tabular}{lcc}
\toprule
...
2. Author opens **new conversation** with fresh Claude
...
1. Author opens **new conversation** with fresh Claude
